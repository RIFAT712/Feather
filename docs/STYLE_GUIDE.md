...
The queue panel's own structure was deliberately rebuilt on 2026-09-09 at the owner's request, so the "queue density" clause above describes what a *normal page redesign* may not touch incidentally -- not a freeze. The panel is now a compact header (titlebar, search, tabs, no stat readout at all -- the four bordered tiles and the one-line pending/accepted/rejected summary were both removed on 2026-09-09 (second revision), leaving a single progress bar in the main-pane flow header as the only queue-wide indicator) over a single list, with `Queue` / `Re-review` / `Other judges` as tabs rather than stacked collapsible sections, and `View as` plus `Group by submitter` behind a gear disclosure. The parts the clause protects were preserved through that change and still are: the review workflow, the iframe preview, the bulk banner and comment panel, and the action bar. See the 2026-09-09 AGENTS.md entry.
