...
`/jury/review` and `/jury/review-v2` are full-screen workspaces. They use the review-specific `--rq-*` tokens and are **light-only**: the dark palette and its toggle were removed on 2026-09-09. `.rq-app` carries `rq-theme-light` unconditionally, so the `.rq-app.rq-theme-light` selectors keep the specificity they were written against -- do not fold them into bare `.rq-app`. As of 2026-09-09 the `--rq-*` light values **are** the Feather core tokens above, mapped by role rather than by name (`--rq-surface` is this route's panel body, so it takes `--feather-surface-alt`; `--rq-surface-alt` is a list row, so it takes `--feather-surface`). Before that the route ran a parallel oklch palette that landed near the Feather values without matching them. Style review-route elements with `--rq-*` tokens only -- no raw hex. Normal application rules must exclude `.rq-app`, and normal page redesigns must not change the review workflow, queue density, iframe preview, bulk comment panel, or review action bar.

The queue panel's own structure was deliberately rebuilt on 2026-09-09 at the owner's request, so the "queue density" clause above describes what a *normal page redesign* may not touch incidentally -- not a freeze. The panel is now a compact header (titlebar, search, tabs) over a single list, carrying no stat readout at all: the four bordered tiles and the one-line pending/accepted/rejected summary were both cut in the 2026-09-09 second revision, leaving a progress bar in the main-pane flow header as the only queue-wide indicator. Judged rows carry no decision tint either -- the sticky Accepted/Rejected block headings do that work. Tabs are with `Queue` / `Re-review` / `Other judges` as tabs rather than stacked collapsible sections, and `View as` plus `Group by submitter` behind a gear disclosure. The parts the clause protects were preserved through that change and still are: the review workflow, the iframe preview, the bulk banner and comment panel, and the action bar. See the 2026-09-09 AGENTS.md entry.
