...
// immune to that.
export async function fetchAllContestLogPages(code, { pageSize = 500, signal, onPage, includeReviews = true } = {}) {
  const items = [];
  let beforeId = null;
  for (;;) {
    const cursor = beforeId !== null ? `&before_id=${beforeId}` : '';
    const reviewsParam = includeReviews ? '' : '&include_reviews=false';
    const res = await fetch(`/api/contests/${code}/log?page_size=${pageSize}${cursor}${reviewsParam}`, { signal });
