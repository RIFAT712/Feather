...
| Frontend | **Vue 3** + **Vite**                 | SPA with Vue Router, Wikimedia Codex UI library, `@tanstack/vue-query` for server-state caching |
...
| GET    | `/api/contests/{code}/log`              | Keyset-paginated activity log, newest first (`before_id`, `page_size` default 200/max 500, `include_reviews` default true, `status` optional filter). `offset` is an explicit opt-in to plain offset pagination instead, for concurrent background-catch-up fetching after a keyset first page — returns `{items, total, page_size, next_before_id, has_more}` | None |
