...
- `backend/jury_panel_store.py` — a **separate SQLite projection DB** (`backend/jury_panel.db`) that mirrors article/review data out of the main DB to serve the paginated jury queue (`/api/jury-panel/contests/{code}/...`) without hitting the primary tables. It tracks a fingerprint (article count, review/submission timestamps, restriction/jury/assignment signatures) to decide when to resync vs. rebalance, and does even, COI-aware load balancing across eligible juries. This is the most change-log-heavy area of the backend — read the relevant AGENTS.md entries before touching jury assignment logic.
...
`backend/.env` holds OAuth credentials, `SESSION_SECRET`, and wiki-replica DB tunnel settings — see `backend/.env.example` for the required keys (including the local SSH tunnel port `4407` for the Bengali replica). Pre-migration DB snapshots are written to `backup/` (gitignored — contains credentials/session data) whenever a schema change is pending; don't delete these without checking they aren't a recent safety net.
